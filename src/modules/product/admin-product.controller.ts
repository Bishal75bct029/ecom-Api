...
export class AdminProductController {
  ...
  async create(@Body() createProductDto: CreateProductDto) {
    ...
    const tags = [...categories.map((category) => category.name), ...newProduct.tags];

    console.log(tags);

    const product = await this.productService.save({ ...newProduct, tags: [...new Set(tags)] });
  ...
  async getAllProducts(@Query('name') name: string) {
    return this.productService.find({
      where: [{ name: ILike(`%${name}%`) }, { tags: ILike(`%${name}%`) }],
  ...
  async update(@Param('id') id: string, @Body() updateProductDto: UpdateProductDto) {
    ...
    const categories = await this.categoryService.find({ where: { id: In(updateProductDto.categoryIds) } });

    const updatedProduct = await this.productService.save({ ...product, ...updateProductDto, categories });
  }
}
